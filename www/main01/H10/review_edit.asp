...
if not rs.eof Then
	...
	If rs("c_strId") <> session("useridid") Then
		Error("수정 권한이 없습니다.")
	End if
%>
...
따라 처벌을 받을 수 있음을 알려드립니다.
	...
	<div class="input_form">
	  <form name="frmRequestForm" method="post" onSubmit="return frmRequestForm_Submit(this);" action="/main01/H10/review_edit_ok.asp?intSeq=<%=request.QueryString("intSeq")%>&gum1=<%=request.QueryString("gum1")%>&gum2=<%=request.QueryString("gum2")%>" enctype="multipart/form-data">
		<table>
			<tbody> 
				<tr>
					<th>작성자</th>
					...
					<td class="call"> <select name="r_bunryu" id="r_bunryu">
					  <option value="f" <%If rs("r_bunryu") = "f" then%>selected<%End if%>>스마일프로</option>
					  <option value="a" <%If rs("r_bunryu") = "a" then%>selected<%End if%>>신세계스마일</option>
					  <option value="b" <%If rs("r_bunryu") = "b" then%>selected<%End if%>>라식</option>
					  <option value="c" <%If rs("r_bunryu") = "c" then%>selected<%End if%>>라섹</option>
					  <option value="d" <%If rs("r_bunryu") = "d" then%>selected<%End if%>>렌즈삽입술</option>
					  <option value="e" <%If rs("r_bunryu") = "e" then%>selected<%End if%>>노안라식</option>
					</select></td>
				</tr>
				<tr>
					<th>원장</th>
...
%>
				...
				<tr> 
					<th>수술 전 시력</th>
					<td class="eye"><input type="text" name="r_eyeforce1-1" value="<%=Split(rs("r_eyeforce1"),"/")(0)%>" placeholder="좌" maxlength="10" /><input type="text" name="r_eyeforce1-2" value="<%=Split(rs("r_eyeforce1"),"/")(1)%>" placeholder="우"maxlength="10" /></td>
					<th>수술 후 시력</th>
					<td class="eye"><input type="text" name="r_eyeforce2-1" value="<%=Split(rs("r_eyeforce2"),"/")(0)%>" placeholder="좌" maxlength="10" /><input type="text" name="r_eyeforce2-2" value="<%=Split(rs("r_eyeforce2"),"/")(1)%>" placeholder="우" maxlength="10" /></td>
				</tr>
				<tr>
					<th>제목</th>
