		...
		<table class="basic_list">
			...
			%>
				<tr>
					...
					<a href="?lasik=I105&seq=<%=getJson.Get(i).seq%>&nPage=<%=request("nPage")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>"><%=getJson.Get(i).title%></a>
					<%else%>
					<a href="?lasik=I103&seq=<%=getJson.Get(i).seq%>&nPage=<%=request("nPage")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>"><%=getJson.Get(i).title%></a>
					<%End if%>
					</td>
