...
function logingogo()
{
	...
	{
		location.href = "/main01/sub.asp?lasik=K101&strBackUrl=<%=strBackUrl%>";
...
<div class="consulting">
  <div class="inner">
	<ul class="top_question"> 
	  ...
	  <li>
		<a href="?lasik=I104"><img src="image/sub/top_icon02.png" alt=""/>문의하기</a>
	  </li>
	</ul>
	<p class="list_txt">
      총 <span><%=intTotalCount%></span>건
      <div class="search_wrap">
		<form name="list_search" id="list_search" method="post" action="?lasik=I102">
    ...
    <table class="basic_list">
	  ...
	  <tr>
		...
		<td>
		  <%If rs2("state") = "N" then%>
		  <a href="?lasik=I105&intSeq=<%=rs2("intSeq")%>&page=<%=request("page")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>"><%=rs2("c_title")%></a>
		  <%else%>
		  <a href="?lasik=I103&intSeq=<%=rs2("intSeq")%>&page=<%=request("page")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>"><%=rs2("c_title")%></a>
		  <%End if%>
		</td>
		<td><%=rs2("c_writer")%></td>
