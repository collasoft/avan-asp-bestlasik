...
<div class="top_tabwrap02"> 
  <div class="inner">
    <ul> 
	  <li <%If request.querystring("sunseo") = "" Or request.querystring("sunseo") = "a" Then%>class="on"<%End if%>><a href="?lasik=G101&sunseo=a">최신순</a></li>
	  <li <%If request.querystring("sunseo") = "b" Then%>class="on"<%End if%>><a href="?lasik=G101&sunseo=b">마감임박순</a></li>
    </ul>
    <div class="search_wrap">
	  <form name="list_search" id="list_search" method="post" action="?lasik=G101">
...
<div class="event"> 
  <div class="inner">
	<ul class="event_list">
	  ...
	  %>
	  <li>
	    <a href="?lasik=G102&intSeq=<%=rs2("intSeq")%>&page=<%=request("page")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>">
		  <%If rs2("e_ing") = "Y" then%>
