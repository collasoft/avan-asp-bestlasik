...
<div class="consulting">
  <div class="inner">
	...
	<div class="input_form">
	  <form name="frmRequestForm" method="post" onSubmit="return frmRequestForm_Submit(this);" action="/main02/H10/bxm_consulting_edit_ok.asp?seq=<%=request.QueryString("seq")%>&gum1=<%=request.QueryString("gum1")%>&gum2=<%=request.QueryString("gum2")%>">
	  <table>
		<tbody> 
		  <tr>
			<th>작성자</th>
	  ...
	  <div class="subbtn_wrap">
		<input type="button" value="목록" class="btn01" onclick="self.location.href='?eye=H102&nPage=<%=request.QueryString("nPage")%>&gum1=<%=request.QueryString("gum1")%>&gum2=<%=request.QueryString("gum2")%>';" /><input type="button" value="등록" class="btn02" onClick="this.form.onsubmit();" />
	  </div>	
	  </form>
	</div>
  </div>
