...
Dim csql, crs
	csql = "select (select count(intSeq) from book2 where c_yymmdd = '" & insert.getFormValue("cyear") & "-" & insert.getFormValue("cmonth") & "-" & insert.getFormValue("cday") & "_" & insert.getFormValue("ctime") & insert.getFormValue("cminut") &"'), (select count(intSeq) from book2 where c_yymmdd like '" & insert.getFormValue("cyear") & "-" & insert.getFormValue("cmonth") & "-" & insert.getFormValue("cday") &"%')"
...
Call insert.settablename("book2")
...
'Call jsAlertMsgUrl("진료예약 등록이 정상적으로 되었습니다.", "/main02/sub.asp?eye=H107")
