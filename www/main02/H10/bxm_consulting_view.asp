  ...
  <div class="inner">
	<div class="input_form">
	  ...
	  <div class="viewbtn_wrap">
		...
		%>
			<input type="button" value="삭제" class="btn01" onclick="location.href='?eye=H109&seq=<%=request.QueryString("seq")%>&nPage=<%=request.QueryString("nPage")%>&gum1=<%=request.QueryString("gum1")%>&gum2=<%=request.QueryString("gum2")%>';" />
			<input type="button" value="수정" class="btn01" onclick="location.href='?eye=H108&seq=<%=request.QueryString("seq")%>&nPage=<%=request.QueryString("nPage")%>&gum1=<%=request.QueryString("gum1")%>&gum2=<%=request.QueryString("gum2")%>';" />
		<%
		End if
		%>

		<input type="button" value="목록" class="btn02" onclick="self.location.href = '?eye=H102&nPage=<%=request.querystring("nPage")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>';" />
	  </div>
	  </form>
	</div>
  </div>
