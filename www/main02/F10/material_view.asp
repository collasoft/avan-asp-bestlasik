	...
	<div class="input_form">
	  <div class="view_wrap"> 
		...
		<ul class="list_box">
		  ...
		  %>
		  <li><a href="?eye=F106&intSeq=<%=rs7("intSeq")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>&page=<%=request.querystring("page")%>"><span>이전</span></a></li>
		  <%rs7.close
			Set rs7=Nothing
			Else
		  %>
		  ...
		  %>
		  <li><a href="?eye=F106&intSeq=<%=rs8("intSeq")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>&page=<%=request.querystring("page")%>"><span>다음</span></a></li>
		  <%rs8.close
			Set rs8=Nothing
			Else
		  %>
	  ...
	  <div class="viewbtn_wrap">
		...
		<input type="button" value="목록" class="btn02" onclick="self.location.href = '?eye=F105&page=<%=request.querystring("page")%>&gum1=<%=request("gum1")%>&gum2=<%=request("gum2")%>';" />
